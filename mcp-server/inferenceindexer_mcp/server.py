...
def _install_reaping_session_manager() -> None:
    """_REAPING_SESSION_MANAGER_PATCH (2026-09-20 OOM fix).

    FastMCP.streamable_http_app() constructs StreamableHTTPSessionManager
    without session_idle_timeout, so HTTP sessions opened by public crawlers
    are tracked forever (13,581 transports in 20 days => OOM on t3.micro).

    Wraps FastMCP.streamable_http_app so the session manager is constructed
    with session_idle_timeout=1800s: idle sessions are terminated and freed
    after 30 minutes of inactivity. Native mcp-sdk feature; we only supply
    the argument FastMCP omits.
    """
    import asyncio
    import mcp.server.fastmcp.server as _fsm
    from mcp.server.streamable_http_manager import StreamableHTTPSessionManager

    _orig_method = _fsm.FastMCP.streamable_http_app

    def _reaping_streamable_http_app(self):
        if self._session_manager is None:
            ...
            # Replicate FastMCP's lazy-start bookkeeping so run() startup
            # (session_manager.run()) behaves exactly as before.
            if hasattr(_fsm.FastMCP, "_session_manager_started"):
                self._session_manager_started = False
        return _orig_method(self)

    _fsm.FastMCP.streamable_http_app = _reaping_streamable_http_app
    del asyncio
...
def main() -> None:
    ...
    if args.transport in ("sse", "streamable-http"):
        ...
        _install_reaping_session_manager()  # _REAPING_SESSION_MANAGER_PATCH
