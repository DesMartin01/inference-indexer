...
async def get_models(
    ...
    query = """
        SELECT m.id, m.name, m.provider, m.tier, m.context_length, m.aa_index_score,
               m.modality, m.is_reasoning, m.creator_country,
    ...
    for row in rows:
        models.append({
            ...
            "modality": row[6],
            "is_reasoning": row[7],
            "creator_country": row[8],
            "input_price_per_m": row[9],
            "output_price_per_m": row[10],
            "blended_price_per_m": row[11],
            "sit_score": row[12],
            "reasoning_multiplier": row[13] if row[13] else 1.0,
            "sit_adjusted_price": row[14],
            ...
            "change_7d": float(row[20]) if row[20] else 0,
        })
